...
SYSTEM_PROMPT = textwrap.dedent("""\
    ...
    2. evidence: 위에서 판단한 llmValue의 근거가 되는 문장을 약관 원문에서 생략·의역 없이 그대로 인용하세요 (단, 아래 "표 형태 예외"에 해당하면 그 규칙을 따르세요). 근거가 되는 문장이 여러 곳에 있으면, llmValue를 가장 직접적으로 뒷받침하는 문장 하나만 인용하세요. llmValue가 null이면 evidence도 null로 하세요.
       - 표 형태 예외: 근거가 <table>/<tr>/<th>/<td> 같은 HTML 표 마크업 안에 있으면, 그 태그를 그대로 인용하지 마세요. 대신 해당 셀들의 텍스트만 뽑아서, 같은 행(row)에 속한 값들끼리는 콤마(,)로 이어붙이고 서로 다른 행은 줄바꿈으로 구분해 표기하세요. 이 경우에도 "생략·의역 없이" 원칙은 동일하게 적용됩니다 - 숫자·단어 등 내용 자체는 하나도 빠짐없이 그대로 유지하고(생략 금지) 태그와 구분자만 바뀌는 것이며(의역 아님), 행 사이의 연관관계(예: 어떤 요금제에 어떤 값이 속하는지)를 절대 섞지 마세요.
    3. page: evidence가 위치한 페이지 번호를 원문의 <!-- PageNumber="N" --> 마커로 판단하세요. 이 마커는 각 페이지 본문 맨 위(내용이 시작되기 직전)에 붙어 있으므로, evidence 바로 위에서 가장 가까운 PageNumber 마커의 값을 사용하세요. evidence가 null이면 page도 null로 하세요.
