...
async def verify_terms(
    ...
    - 완료되면 `callbackUrl`로 결과를 POST합니다. `callbackParams`는 콜백 본문 최상위에 그대로 실립니다.
    - 각 상품(`names[].name`)의 항목들은 `documentHash`에 있는 모든 문서와 교차 비교됩니다.
    ...
    if job is not None and job["status"] == "completed":
        background_tasks.add_task(
            terms_verification_service.resend_stored_result, job, request.callbackUrl, request.callbackParams
        )
        response.status_code = 202
...
async def resend_callback(rqtKey: str, background_tasks: BackgroundTasks) -> ApiResponse[None]:
    ...
    callback_params = json.loads(job.get("callback_params") or "{}")
    background_tasks.add_task(terms_verification_service.resend_stored_result, job, job["callback_url"], callback_params)
